...
- DESIGN known-limit coverage: 不支持删除、`EXPANSION 1` 查询成本、AOF rewrite OOM skip-key、RedisBloom/Redis 8 同实例互斥、配置差异已在 Stage 13 worker recheck 中按报告覆盖项补齐，证据 `.codex/gemini-bloom-audit/v6/agents/stage13_worker_recheck/stage_result.md`。
...
| RedisBloom module-load 配置差异对照复跑 | NOT_VERIFIED | 本次 worker 只按 DESIGN、gemini 配置测试说明和静态解析核对补齐报告覆盖；未新启动 RedisBloom 实例验证 `EXPANSION` / `CF_MAX_EXPANSIONS` load arg 行为 | `.codex/gemini-bloom-audit/v6/agents/stage13_worker_recheck/stage_result.md` |
